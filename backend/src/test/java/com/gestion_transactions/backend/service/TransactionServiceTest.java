...
public class TransactionServiceTest {
    ...
    public void testWithdraw_Succes() {
        ...
        // Act : On retire 200$
        transactionService.withdraw(1L, 200.0, "Alice Dupont", "0700000000");
    ...
    public void testDeposit_Succes() {
        ...
        bank.setWithdrawalFee(0.0);
        bank.setDepositFee(0.0);
        bank.setMinimumDepositAmount(0.0);
        bank.setActive(true);
        compte.setBank(bank);
        ...
        // Act : On dépose 150$
        transactionService.deposit(2L, 150.0, "Bob Martin", "0700000000");
    ...
    @Test
    public void testDeposit_MontantInferieurSeuil_DoitEchouer() {
        // Arrange : Un compte avec un minimum de dépôt de 200
        Account compte = new Account();
        compte.setId(4L);
        Bank bank = new Bank();
        bank.setId(4L);
        bank.setWithdrawalFee(0.0);
        bank.setDepositFee(0.0);
        bank.setMinimumDepositAmount(200.0);
        bank.setActive(true);
        compte.setBank(bank);
        compte.setBalance(100.0);
        ...
        User user = new User();
        user.setId(4L);
        user.setName("David Leroy");
        user.setPhone("0700222333");
        compte.setUser(user);

        when(accountRepository.findById(4L)).thenReturn(Optional.of(compte));

        Exception exception = assertThrows(RuntimeException.class, () -> {
            transactionService.deposit(4L, 150.0, "David Leroy", "0700222333");
        });

        assertTrue(exception.getMessage().contains("montant du dépôt doit être au moins"));
    }

    @Test
    public void testDeposit_InfoUtilisateurNonConforme_DoitEchouer() {
        Account compte = new Account();
        compte.setId(3L);
        Bank bank = new Bank();
        bank.setId(3L);
        ...
        User user = new User();
        user.setId(3L);
        user.setName("Carole Bernard");
        user.setPhone("0700111222");
        compte.setUser(user);

        when(accountRepository.findById(3L)).thenReturn(Optional.of(compte));

        Exception exception = assertThrows(RuntimeException.class, () -> {
            transactionService.deposit(3L, 50.0, "Carole B.", "0700000000");
        });

        assertTrue(exception.getMessage().contains("informations fournies ne correspondent pas"));
    }
    ...
    public void testWithdraw_LimiteQuotidienneDepassee_DoitEchouer() {
        ...
        Exception exception = assertThrows(RuntimeException.class, () -> {
            transactionService.withdraw(1L, 300.0, "Alice Dupont", "0700000000");
        });
    ...
    public void testWithdraw_LimiteQuotidienneRespectee_DoitReussir() {
        ...
        transactionService.withdraw(1L, 400.0, "Alice Dupont", "0700000000");
